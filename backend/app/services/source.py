...
class SourceService:
  ...
  async def create_source(self, data:SourceCreate) -> Source:
    ...
    """
    logger.info("Creating source", source_type=data.type, workspace_id=str(data.workspace_id))
  ...
  async def update_source(self, source_id: UUID, data: SourceUpdate) -> Source:
      ...
      logger.debug("Update data", source_id=str(source_id), fields=list(update_data.keys()))
